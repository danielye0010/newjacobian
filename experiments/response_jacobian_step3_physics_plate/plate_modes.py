"""Analytical plate modes for Step 3 physics-response diagnostics."""
...
def mass_orthonormalize(V: np.ndarray, weights_dof: np.ndarray) -> np.ndarray:
    ...
    for j in range(V.shape[1]):
        ...
        if norm < 1e-12:
            raise ValueError(f"Mode {j} is dependent")
        Q.append(v / norm)
...
def create_plate_modes(mesh: PlateMesh) -> Tuple[np.ndarray, Dict[str, object]]:
    ...
    return B, {
        ...
        "mode_descriptions": [
            ...
            "z quadratic bowl in x",
            "z quadratic bowl in y",
            "in-plane x shrink/stretch",
            "in-plane y shrink/stretch",
            "z twist normalized x*y",
        ],
    }
